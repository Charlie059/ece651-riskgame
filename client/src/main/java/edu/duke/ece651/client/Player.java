...
>>>>>>> 42-implement-action-handler
  ...
  // Total number of unites depend on num of Territories
  public void initializeDeployment() {
    totalDeployment = new HashMap<>();
    totalDeployment.put(1, myTerritories.size() * 3);// add 3 * territory# units to level 1
  }
    ...
    do {
      ...
      for (String terrName : wholeMap.getTerritoryList().keySet()) {
        ...
        if (wholeMap.getTerritoryList().get(terrName).getOwner() != this.id) {
          ...
          out.println("   Units:");
          for (Integer i : wholeMap.getTerritoryList().get(terrName).getUnits().keySet()) {
            out.println("       Level " + i + " Units: " + wholeMap.getTerritoryList().get(terrName).getUnits().get(i));
          }
          out.println();
        }
      }
...
>>>>>>> 42-implement-action-handler
  ...
  /**
   * send their units to an adjacent territory controlled by a different player,
   * in an attempt to gain control over that territory.
   * 
   * @param attackUnits
   * @param from_name
   * @param to_name
